...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a database of files for the given directory."
    )
    ...
    parser.add_argument(
        "-u", action="store_true", default=False, dest="assign_uid", help="Assign UIDs."
    )
    parser.add_argument(
...
def check_current_fs_status(
    master: list[Entries],
) -> Tuple[list[Entries], list[Entries]]:
    ...
    for item in master[:]:
        ...
        # Entry has changed size, bail so user can investigate.
            ...
            ml.exit_error(
                f"{item.name} has changed size from {item.current_size} to {size}."
            )
...
def search_file_path(
    master: list[Entries], quarentine: list[Entries], file_path: str
) -> Tuple[int, int]:
    ...
    while found:
        ...
        if found and (
            os.path.join(master[fp_index].path, master[fp_index].name) == file_path
        ):
            return (MASTER, fp_index)
        start = fp_index
    found, fp_index = ml.check_inode_in_path(
        master, os.path.dirname(file_path), fp_stat.st_ino
    )
    if found and (master[fp_index].current_size == fp_stat.st_size):
        return (MASTER, fp_index)
    # Look in quarentine by inode and size, then size and mtime.
    found, fp_index = ml.check_inode_in_path(
        quarentine, os.path.dirname(file_path), fp_stat.st_ino
    )
    ...
    while found:
        ...
        if found and (
            quarentine[fp_index].date
            == datetime.datetime.fromtimestamp(
                fp_stat.st_mtime, tz=datetime.timezone.utc
            )
        ):
            return (QUARENTINE, fp_index)
        start = fp_index
...
def process_targets(
    master: list[Entries], quarentine: list[Entries], target_paths: list[str]
) -> list[Entries]:
